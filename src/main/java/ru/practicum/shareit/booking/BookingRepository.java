...
public interface BookingRepository extends JpaRepository<Booking, Integer> {
    ...
    @Query(value = "select * from bookings where item_id = ?1 AND start_date = (SELECT MAX(start_date) from bookings where item_id = ?1 AND start_date < now())", nativeQuery = true)
    Optional<Booking> findLastBooking(Integer itemId);
    @Query(value = "select * from bookings where item_id = ?1 AND start_date = (SELECT MIN(start_date) from bookings where item_id = ?1 AND start_date > now())", nativeQuery = true)
